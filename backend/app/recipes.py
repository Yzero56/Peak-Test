...
키가 없거나 요청이 실패하면 빈 리스트를 반환한다 — 호출부(app_api)가 이걸 보고
...
def fetch_recipes() -> list[dict]:
    ...
    if not settings.food_safety_api_key:
        return []
    ...
    url = f"{API_BASE}/{settings.food_safety_api_key}/{SERVICE_ID}/json/1/{FETCH_COUNT}"
    try:
