"""screen_capture_sender.py — 화면 일부(주로 QuickTime의 iPhone 미러링 창)를 캡처해서
phone_mirror_relay.py로 WebSocket 전송한다.
...
준비:
  ...
  3. 그 QuickTime 창을 화면 한쪽에 고정해두고, 이 스크립트의 --region으로 그
     창의 좌표/크기를 알려준다(모르면 --monitor로 화면 전체를 일단 캡처해도 됨).

실행:
  pip install mss pillow websockets
  python dev/screen_capture_sender.py --relay ws://localhost:9600/ingest
  # 특정 영역만(QuickTime 창 좌표를 알 때, macOS 스크린샷 도구(Cmd+Shift+4)로
...
def main() -> None:
    ...
    ap.add_argument("--region", default=None,
                     help="left,top,width,height (QuickTime 창 좌표) — 안 주면 --monitor 전체 캡처")
