...
FRAME_LOG_MAX_S = 25.0  # 세션 중 프레임 기록을 몇 초치 들고 있을지
# 원래 8초였는데, 실측해보니 문 여는 데만 3.8~4초 걸리고 물건 넣고 빼는 동작까지
# 하면 세션이 8초를 쉽게 넘겨서, "문 막 열렸을 때의 안정적인 프레임"이 버퍼에서
# 밀려나 사라지고 닫히는 중/닫힌 프레임만 후보로 남는 문제가 실제로 발생했다
# (after 사진에 닫힌 문이 찍힘 — 육안 확인). 넉넉하게 25초로 늘림.
...
#   th=0.5  -> 노출 64/74, 그중 정확도 76.6%
#   th=0.55 -> 노출 57/74, 그중 정확도 78.9%
#   th=0.65 -> 노출 45/74, 그중 정확도 84.4%(너무 많이 숨겨짐 — 이전 값)
# 이후 110장짜리 별도 held-out 세트로 재확인한 스윕(TTA/앙상블 실험 중 부산물)도
# 같은 지점에서 최고점을 보임:
#   th=0.4/0.45 -> 노출 110/110, 정확도 74.5%
#   th=0.5  -> 노출 107/110(숨김 3),  정확도 75.7%
#   th=0.55 -> 노출  95/110(숨김 15), 정확도 78.9%  (채택 — 두 세트 모두 여기서 최고)
#   th=0.6  -> 노출  88/110(숨김 22), 정확도 77.3%
#   th=0.65 -> 노출  78/110(숨김 32), 정확도 75.6%(너무 많이 숨겨짐)
# 두 번 다 0.55에서 "노출 vs 숨김" 균형이 제일 좋아서 0.5 -> 0.55로 올림.
UNCERTAIN_CONFIDENCE_TH = 0.55
