// 냉장고 용기 인식 프로젝트 - 1단계: 데이터 수집기
// 보드가 자체 핫스팟(AP)이 되고, 라벨을 붙여 사진을 캡처해서
// "<라벨>.<번호>.jpg" 형식으로 ZIP 다운로드 (추후 YOLO 학습용 데이터셋)
//
// Wi-Fi "ESP32-Camera" (pw 12345678)에 연결 후 http://192.168.4.1 접속
#include <WiFi.h>
#include <WebServer.h>
...
button.quick{background:#333;border:2px solid #4c8bf5;font-size:15px;padding:7px 12px}
button.quick.active{background:#4c8bf5}
...
<h2>냉장고 용기 데이터 수집기</h2>
<img id="cam" src="/jpg">
<div id="quick">
<button class="quick" onclick="setLabel('컵')">컵</button>
<button class="quick" onclick="setLabel('락앤락')">락앤락</button>
<button class="quick" onclick="setLabel('원형반찬통')">원형 반찬통</button>
</div>
<div>
<input id="label" placeholder="라벨 (예: 컵)" value="컵">
...
function zipAll(){
  ...
  a.href=URL.createObjectURL(blob); a.download='container_dataset.zip'; a.click();
...
void setup() {
  ...
  WiFi.mode(WIFI_AP);
  WiFi.softAP("ESP32-Camera", "12345678", 1);
