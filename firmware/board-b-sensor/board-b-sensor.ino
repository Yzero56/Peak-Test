...
//
// [통합 변경] 원래는 4번 파트 자체 백엔드의 /api/devices/{id}/sensors(기기 토큰 인증)로
// 보고했지만, 백엔드를 kang 브랜치의 /api/v1 백엔드로 단일화하면서 그쪽 스키마
// (SensorReadingCreate: device_id/temperature/humidity/gas_resistance_ohm/door_open)에
// 맞춰 보낸다. kang 백엔드는 현재 개발 단계라 인증이 없어 토큰 헤더도 제거했다.
void reportSensors() {
  if (WiFi.status() != WL_CONNECTED) return;

  HTTPClient http;
  String url = String(BACKEND_BASE_URL) + "/api/v1/sensor-readings";
  http.begin(url);
  ...
  if (envReadingValid) {
    body += ",\"temperature\":" + String(lastTemperatureC, 2);
    body += ",\"humidity\":" + String(lastHumidityPct, 2);
...
// [통합 변경] kang 백엔드에는 4번 파트 자체 백엔드에 있던 기기 등록/하트비트
// (last_seen_at, 기기별 /stream·/capture 주소 기억) 개념이 아직 없어서 보낼 곳이
// 없다 — 이 보드의 /stream, /capture는 지금은 로컬 LAN에서 IP로 직접 접속하는
// 용도로만 남겨두고, 하트비트 전송은 끈다. 백엔드가 "지금 스캔하기"를 누른 순간
// 이 보드의 /capture를 당겨가는 pull 방식을 그대로 쓸지, 아니면 이 보드가 캡처한
// 프레임을 kang의 POST /api/v1/food-images로 직접 올리는 push 방식으로 바꿀지는
// 아직 결정되지 않은 부분 — INTEGRATION_NOTES.md 참고.
void sendHeartbeat() {
  // no-op: kang 백엔드에 기기 등록/하트비트 엔드포인트가 생기면 여기에 다시 연결.
}
